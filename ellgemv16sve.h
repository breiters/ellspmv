...
#pragma omp for
  for (int i = 0; i < num_rows; i++) {
    ...
    if (i < num_rows - SW_PREFETCH_DIST_L1) {
      ...
      svprfd_gather_index(p64_all, x, svunpklo(cidxv_pf), SV_PLDL1KEEP);
      // extend high half into index vector and prefetch (col8-col15)
      svprfd_gather_index(p64_all, x, svunpkhi(cidxv_pf), SV_PLDL1KEEP);
    }

#ifdef A64FXCPU
// cached colidx no more needed -> assign sector 1
#pragma statement scache_isolate_assign colidx
#endif /* A64FXCPU */
    // load 16 colidx values
    svint32_t cidxv = svld1(svptrue_b32(), &colidx[i * 16]);
#ifdef A64FXCPU
#pragma statement end_scache_isolate_assign colidx
#endif /* A64FXCPU */
    ...
    // load x[col0,...,col7]
    svfloat64_t xv0 = svld1_gather_index(p64_all, x, cidxvlo);
    // load x[col8,...,cold15]
    svfloat64_t xv1 = svld1_gather_index(p64_all, x, cidxvhi);

    // load a[0-7]
    svfloat64_t av0 = svld1(p64_all, &a[i * 16 + 0]);
    // load a[8-15]
    svfloat64_t av1 = svld1(p64_all, &a[i * 16 + 8]);

    // a <- a[0-7] * x[col0,...,col7]
    av0 = svmul_z(p64_all, av0, xv0);
    // a <- a + a[8-15] * x[col8,...,col15]
    av1 = svmla_z(p64_all, av0, av1, xv1);

    // reduce a*x into y
    float64_t y0 = svaddv(p64_all, av1);
